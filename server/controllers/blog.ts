  ...
  if (req.files) {
    if (Array.isArray(req.files)) {
      ...
      for (const key of Object.keys(req.files)) {
        const entry = (req.files as Record<string, Express.Multer.File[]>)[key];
...
export const editBlogHandler = async (req: Request, res: Response) => {
  ...
  if (!blogId) {
    return res
      .status(400)
      .json({ message: "blog entry not found or may have been moved to a different url" });
  }
  try {
    const { title, excerpt, tags, content, isDraft, imageURL } = req.body;
    ...
    const bodyImageURLs = Array.isArray(imageURL) ? imageURL : [];
    const fileImageURLs = files.length ? await uploadImages(files) : [];
    const finalImageURLs = bodyImageURLs.length
      ? [...bodyImageURLs, ...fileImageURLs]
      : [...(reqBlog?.imageURL || []), ...fileImageURLs];
