  ...
  try {
    ...
    try {
      ...
      if (roomDoc && io) {
        ...
        socketsInRoom.forEach((socket: unknown) => {
          const s = socket as { userId?: string };
          if (s.userId) {
            connectedUserIds.add(s.userId.toString());
          }
        });

        const pushTokens: string[] = [];
        roomDoc.members.forEach((member: { _id: { toString: () => string }; expoPushToken?: string }) => {
          const memberId = member._id.toString();
          // Only send to users who are NOT the sender AND NOT currently connected
          if (memberId !== sender?.toString() && !connectedUserIds.has(memberId) && member.expoPushToken) {
