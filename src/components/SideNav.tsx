...
import { MessagesSquare, Users, User, ChevronLeft, ChevronRight, type LucideIcon } from "lucide-react";
...
export function SideNav() {
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center pt-5 pb-4 ${collapsed ? "justify-center px-0" : "px-4 gap-3"}`}>
        <img src={wwhsLogo} alt="" className="size-9 shrink-0 rounded-xl object-contain shadow-sm" />
        {!collapsed && (
          <span className="text-sm font-semibold leading-tight text-white/90 truncate">{BRAND_NAME}</span>
        )}
      </div>
